#!/usr/bin/env python3
"""Run frozen v1.5 once: deterministic mapping and bounded evidence observations."""
...
W=Path(__file__).resolve().parent; P=W.parents[1]; V14=W.parent/'evidence-gated-v1.4'; V13=W.parent/'evidence-gated-v1.3'; V12=W.parent/'evidence-gated-v1.2'
R=P/'results'/'evidence-gated-v1.5'; META=W/'workflow-v1.5.meta.json'; CERT=W/'certification'/'contract-certification-v1.5.json'; PLAN=W/'certification'/'plan-v1.5.json'; V13CERT=V13/'certification'/'contract-certification-v1.3.json'; RULEBOOK=P/'rulebooks'/'coverage-evidence-v1.0'/'rulebook.json'
...
def verify(meta:dict[str,Any]):
    ...
    if bad or meta['status']!='frozen_pre_regression' or meta['regression_run_count']!=0: raise RuntimeError('Frozen v1.5 verification failed: '+','.join(bad))
    c=load_json(CERT)
    if c.get('status')!='completed' or c.get('completed_call_count')!=1 or sha256_file(CERT)!=meta['evidence_certification_sha256']: raise RuntimeError('Changed evidence certification incomplete or stale')
    rec=c['records'][0]
    if rec['schema_sha256']!=sha256_file(W/'schemas/evidence-observation-stage.schema.json') or rec['prompt_sha256']!=sha256_file(W/'prompts/evidence-observation-stage-v1.5.md'): raise RuntimeError('Changed evidence contract differs from certification')
    lineage=load_json(V13CERT); src={x['stage']:x for x in lineage['records']}['source_support']
    if sha256_file(V13CERT)!=meta['v1_3_certification_lineage_sha256'] or src['schema_sha256']!=sha256_file(W/'schemas/source-support-stage.schema.json') or src['prompt_sha256']!=sha256_file(W/'prompts/source-support-stage-v1.3.md'): raise RuntimeError('Inherited source contract differs')
...
def main():
    ...
    profile=load_json(P/'profiles/synthetic-investment-wealth-institution-v1.0.json'); catalogue={x['control_id'] for x in profile['controls']}; rb=load_json(RULEBOOK); validate_rulebook(rb,catalogue)
    if len(cases)!=24: raise RuntimeError('Frozen cases changed')
    if a.preflight:
        if R.exists(): raise RuntimeError('v1.5 results already exist')
        print('Evidence-gated v1.5 preflight passed: approved rulebook, inherited source contract 1/1, changed evidence contract 1/1, 24 cases and at most 24 calls. No API call made.'); return 0
    if R.exists(): raise RuntimeError('v1.5 already started; refusing overwrite')
    ...
    run_id=f'evidence-gated-v1.5-{uuid.uuid4()}'; state_path=R/'run-state.json'; ledger=R/'stage-ledger.jsonl'; out_path=R/'predictions.json'
    state={'run_id':run_id,'status':'running','started_at':utc_now(),'completed_at':None,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.5','model':meta['model'],'temperature':0,'case_count':24,'maximum_api_call_count':24,'requested_api_call_count':0,'semantically_valid_api_call_count':0,'completed_case_count':0,'quarantined_case_count':0,'active_case_id':None,'active_stage':None,'tuning_after_observation':False,'retry_count':0,'error':None}; write_json(state_path,state)
    ...
    for pos,case in enumerate(cases,1):
        try:
            ...
            decision,trace=compose_final_decision(case,source,mapping,observations,approved,*policies,profile,rb); final.validate(decision)
    ...
    status='completed_with_quarantine' if quarantines else 'completed'; write_json(out_path,{'run_id':run_id,'status':status,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.5','workflow_meta_sha256':sha256_file(META),'model_requested':meta['model'],'temperature':0,'run_attempt_count':1,'requested_api_call_count':state['requested_api_call_count'],'retry_count':0,'tuning_after_observation':False,'case_count':24,'predictions':predictions,'quarantines':quarantines}); state['status']=status; state['completed_at']=utc_now(); write_json(state_path,state); print(f'Completed one frozen v1.5 regression: completed={len(predictions)} quarantined={len(quarantines)} calls={state["requested_api_call_count"]}'); return 0
