#!/usr/bin/env python3
"""Run frozen v1.6 once: deterministic mapping and bounded evidence observations."""
...
R=P/'results'/'evidence-gated-v1.6'; META=W/'workflow-v1.6.meta.json'; V15CERT=V15/'certification'/'contract-certification-v1.5.json'; RULEBOOK=P/'rulebooks'/'coverage-evidence-v1.0'/'rulebook.json'; COMPOSITION=P/'policies'/'assurance-composition-v1.0'/'policy.json'
...
def verify(meta:dict[str,Any]):
    ...
    if bad or meta['status']!='frozen_pre_regression' or meta['regression_run_count']!=0: raise RuntimeError('Frozen v1.6 verification failed: '+','.join(bad))
...
def main():
    ...
    if a.preflight:
        if R.exists(): raise RuntimeError('v1.6 results already exist')
        print('Evidence-gated v1.6 preflight passed: approved rulebook and composition policy, inherited provider contracts 2/2, 24 cases and at most 24 calls. No API call made.'); return 0
    if R.exists(): raise RuntimeError('v1.6 already started; refusing overwrite')
    ...
    run_id=f'evidence-gated-v1.6-{uuid.uuid4()}'; state_path=R/'run-state.json'; ledger=R/'stage-ledger.jsonl'; out_path=R/'predictions.json'
    state={'run_id':run_id,'status':'running','started_at':utc_now(),'completed_at':None,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.6','model':meta['model'],'temperature':0,'case_count':24,'maximum_api_call_count':24,'requested_api_call_count':0,'semantically_valid_api_call_count':0,'completed_case_count':0,'quarantined_case_count':0,'active_case_id':None,'active_stage':None,'tuning_after_observation':False,'retry_count':0,'error':None}; write_json(state_path,state)
    ...
    status='completed_with_quarantine' if quarantines else 'completed'; write_json(out_path,{'run_id':run_id,'status':status,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.6','workflow_meta_sha256':sha256_file(META),'model_requested':meta['model'],'temperature':0,'run_attempt_count':1,'requested_api_call_count':state['requested_api_call_count'],'retry_count':0,'tuning_after_observation':False,'case_count':24,'predictions':predictions,'quarantines':quarantines}); state['status']=status; state['completed_at']=utc_now(); write_json(state_path,state); print(f'Completed one frozen v1.6 regression: completed={len(predictions)} quarantined={len(quarantines)} calls={state["requested_api_call_count"]}'); return 0
