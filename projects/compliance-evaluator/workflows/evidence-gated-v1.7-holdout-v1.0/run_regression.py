...
R=P/'results'/'evidence-gated-v1.7-holdout-v1.0'; META=W/'workflow-holdout-v1.0.meta.json'; V15CERT=V15/'certification'/'contract-certification-v1.5.json'; RULEBOOK=P/'rulebooks'/'coverage-evidence-v1.0'/'rulebook.json'; COMPOSITION=P/'policies'/'assurance-composition-v1.0'/'policy.json'
...
INPUTS=(P/'datasets/holdout-v1.0/inputs-aml-cft.json',P/'datasets/holdout-v1.0/inputs-market-conduct.json')

def verify(meta:dict[str,Any]):
    bad=[p for p,h in meta['artefact_sha256'].items() if not (P/p).exists() or sha256_file(P/p)!=h]
    if bad or meta['status']!='frozen_pre_holdout' or meta['holdout_run_count']!=0: raise RuntimeError('Frozen v1.7 verification failed: '+','.join(bad))
...
def main():
    ...
    if len(cases)!=16: raise RuntimeError('Frozen cases changed')
    if a.preflight:
        if R.exists(): raise RuntimeError('v1.7 results already exist')
        print('Evidence-gated v1.7 preflight passed: approved rulebook and composition policy, inherited provider contracts 2/2, 16 cases and at most 16 calls. No API call made.'); return 0
    ...
    run_id=f'evidence-gated-v1.7-holdout-v1.0-{uuid.uuid4()}'; state_path=R/'run-state.json'; ledger=R/'stage-ledger.jsonl'; out_path=R/'predictions.json'
    state={'run_id':run_id,'status':'running','started_at':utc_now(),'completed_at':None,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.7','model':meta['model'],'temperature':0,'case_count':16,'maximum_api_call_count':16,'requested_api_call_count':0,'semantically_valid_api_call_count':0,'completed_case_count':0,'quarantined_case_count':0,'active_case_id':None,'active_stage':None,'tuning_after_observation':False,'retry_count':0,'error':None}; write_json(state_path,state)
    ...
    status='completed_with_quarantine' if quarantines else 'completed'; write_json(out_path,{'run_id':run_id,'status':status,'dataset_id':meta['dataset_id'],'dataset_version':meta['dataset_version'],'workflow_id':meta['workflow_id'],'workflow_version':'1.7','workflow_meta_sha256':sha256_file(META),'model_requested':meta['model'],'temperature':0,'run_attempt_count':1,'requested_api_call_count':state['requested_api_call_count'],'retry_count':0,'tuning_after_observation':False,'case_count':16,'predictions':predictions,'quarantines':quarantines}); state['status']=status; state['completed_at']=utc_now(); write_json(state_path,state); print(f'Completed one frozen v1.7 holdout: completed={len(predictions)} quarantined={len(quarantines)} calls={state["requested_api_call_count"]}'); return 0
