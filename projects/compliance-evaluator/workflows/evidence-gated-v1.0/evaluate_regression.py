#!/usr/bin/env python3
"""Score one evidence-gated regression and compare it with the preserved simple baseline."""
...
def build_comparison(
    evaluation: dict[str, Any], baseline: dict[str, Any], gates: dict[str, Any]
) -> dict[str, Any]:
    field_names = (
    ...
    for field in field_names:
        ...
        fields[field] = {
            ...
            "evidence_gated_accuracy": current_accuracy,
            "delta": delta(current_accuracy, baseline_accuracy),
        }
    binary = {}
...
def build_report(evaluation: dict[str, Any], comparison: dict[str, Any]) -> str:
    ...
    current = comparison["end_to_end_exact"]["evidence_gated"]
    lines = [
        ...
        "|---|---:|---:|---:|",
        f"| End-to-end exact | {percent(baseline['accuracy'])} | {percent(current['accuracy'])} | {percent(comparison['end_to_end_exact']['accuracy_delta'])} |",
    ]
    for field, label in (
        ...
        lines.append(
            ...
            f"{percent(metric['evidence_gated_accuracy'])} | {percent(metric['delta'])} |"
        )
        ...
        predictions.append(
            {
                ...
                "case_stage": stages[case_id],
                "prediction": prediction,
                "reconciliation": {"passed": True, "manual_correction": False},
            }
        )
    return {
        "run_id": "evidence-gated-self-test",
        "status": "completed",
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("regression_evaluator_self_test_passed: 24/24 exact and 8/8 gates")
        return 0
    ...
    gates = acceptance_gates(payload, evaluation)
    comparison = build_comparison(evaluation, baseline, gates)
