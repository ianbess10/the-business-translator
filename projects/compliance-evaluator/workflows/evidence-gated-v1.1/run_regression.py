#!/usr/bin/env python3
"""Run frozen Evidence-Gated Decision Pipeline v1.1 once against input-only cases."""
...
def approved_obligation_payload(record: dict[str, Any]) -> dict[str, Any]:
    return {
        "obligation_id": record["obligation_id"],
        "lifecycle_status": record["lifecycle_status"],
        "source": {
            "source_id": record["source"]["source_id"],
            "source_status": record["source"]["source_status"],
            "locator": record["source"]["locator"],
        },
        "classification": record["classification"],
        "obligation": {
            "atomic_summary": record["obligation"]["atomic_summary"],
            "evidence_or_reporting_requirements": record["obligation"][
                "evidence_or_reporting_requirements"
            ],
            "support_status": record["obligation"]["support_status"],
        },
        "applicability": record["applicability"],
        "operating_impact": record["operating_impact"],
...
def assurance_payload(
    ...
    return {
        "case_id": case["case_id"],
        "approved_obligation_record": approved_obligation_payload(
            approved_obligation
        ),
        "candidate_statement": case["candidate_statement"],
        "source_excerpt_or_fact": case["source_excerpt_or_fact"],
...
def main() -> int:
    ...
    source_policy = load_json(
        WORKFLOW_DIR / "policies" / "source-transition-policy-v1.1.json"
    )
    applicability_policy = load_json(
        WORKFLOW_DIR / "policies" / "applicability-policy-v1.1.json"
    )
    evidence_policy = load_json(
        WORKFLOW_DIR / "policies" / "evidence-decision-policy-v1.1.json"
    )
    ...
    source_prompt = (
        WORKFLOW_DIR / "prompts" / "source-obligation-stage-v1.1.md"
    ).read_text(encoding="utf-8")
    assurance_prompt = (
        WORKFLOW_DIR / "prompts" / "control-evidence-stage-v1.1.md"
    ).read_text(encoding="utf-8")
    ...
    if len(approved_records) != entered_count:
        raise RuntimeError("Approved-obligation fixture count does not match assurance entries")
    source_stage_count = len(inputs) - entered_count
    if source_stage_count != meta["source_obligation_case_count"]:
        raise RuntimeError(
            "Source-stage case count changed: "
            f"metadata={meta['source_obligation_case_count']}, calculated={source_stage_count}"
        )
    if entered_count != meta["approved_assurance_case_count"]:
        raise RuntimeError(
            "Approved-assurance case count changed: "
            f"metadata={meta['approved_assurance_case_count']}, calculated={entered_count}"
        )
    expected_calls = source_stage_count + entered_count
    ...
    if args.preflight:
        if RESULTS_DIR.exists():
            raise RuntimeError(
                "Evidence-gated v1.1 result directory already exists; pre-regression boundary is not clear"
            )
        print(
            f"Evidence-gated v1.1 preflight passed: {len(inputs)} cases, "
            f"{source_stage_count} source assessments, {entered_count} approved assurance "
            f"assessments, {expected_calls} future API calls, model {meta['model']}. "
            "No API call made."
        )
        return 0
    ...
    if marker_path.exists() or predictions_path.exists():
        raise RuntimeError(
            "Evidence-gated v1.1 regression already started or completed; refusing to overwrite it"
        )

    run_id = f"evidence-gated-v1.1-{uuid.uuid4()}"
    ...
    try:
        for index, case in enumerate(inputs, start=1):
            ...
            if assurance_entered:
                ...
                stage_two, assurance_api = request_stage(
                    ...
                    assurance_prompt,
                    assurance_format,
                    assurance_payload(case, approved_obligation, profile),
                )
            ...
            prediction_records.append(
                {
                    ...
                    "stage_outputs": {
                        ...
                        "approved_obligation_authority": (
                            ...
                            else approved_obligation_payload(approved_obligation)
                        ),
                    },
        ...
        write_json(marker_path, run_state)
        print(f"Completed one frozen evidence-gated v1.1 regression: {predictions_path}")
        return 0
