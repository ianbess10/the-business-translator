...
The approved [Coverage and Evidence Rulebook v1.0](rulebooks/coverage-evidence-v1.0/README.md) and separate [Evidence-Gated Decision Pipeline v1.5](workflows/evidence-gated-v1.5/README.md) implement that decision. The rulebook contains reviewed obligation elements, complementary control-coverage assertions and typed evidence requirements, with explicit prohibitions on benchmark identities, labels and case exceptions. Mapping completeness is deterministic and uses no model call; the model is limited to bounded evidence observations. Offline integrity, provider-subset, routing, ledger, quarantine and evaluator tests passed before freeze. The unchanged source contract retained exact certification lineage, and the changed evidence-observation contract passed one non-benchmark provider certification.
...
The completed [v1.6 case-level failure analysis](analysis/evidence-gated-v1.6/README.md) confirms that `CON-008` is correct under the approved rulebook and composition policy: partial coverage and design deficiency coexist, requiring both mapping review and control design change. The mismatch arises because benchmark authority v1.1 migrated only four earlier disputes and silently inherited `CON-008`'s legacy single-axis fields. The separately versioned [v1.7 decision](analysis/evidence-gated-v1.6/v1.7-decision.md) therefore requires an independent audit of all eight entered-assurance cases, a complete self-contained authority v1.2 and an evaluator that fails closed on missing authority. It retains the v1.6 runtime by default and does not implement, execute or score v1.7.
